...
{
	double elapse = static_cast<double>(end - start) / CLOCKS_PER_SEC * 1000.0;
	return (elapse);
}
...
void				PmergeMe::sortListContainer( void )
{
	...
	std::cout << "List is " << (PmergeMe::isListSorted(_list) ? "sorted" : "not sorted") << std::endl;
}
...
void				PmergeMe::sortVectorContainer( void )
{
	...
	std::cout << PmergeMe::measureTime(start, end) << " us" << std::endl;
	std::cout << "Vector is " << (PmergeMe::isVectorSorted(_vector) ? "sorted" : "not sorted") << std::endl;
}
